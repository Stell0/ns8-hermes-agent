...
The Hermes wrapper patches the upstream dashboard sources at container start, rebuilds the web bundle locally, then injects both the runtime `BASE_URL` and an HTML `<base href>` into the served `index.html`.
