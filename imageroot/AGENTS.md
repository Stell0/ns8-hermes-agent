...
- Keep shared smarthost discovery in `discover-smarthost`, keep per-agent env and secrets generation in `sync-agent-runtime`, and keep managed home seeding in `seed-agent-home`. If you change generated file names or payload fields, update the actions, event handler, unit file, tests, UI, and docs together.
